...
//      data.Inco2L = this.getView().byId("inco1").getSelectedItem().getProperty("text");
    ...
    //  if(this.decimalFormat == 'X'){
        menge = menge.toString().replace(/,/g,"\.");
    //  }
  ...
  //    if(this.decimalFormat == 'X'){
        netpr = netpr.toString().replace(/,/g,"\.");
  //    }
...
//      utils.setInitInput(this.oWerks);
      //1. buyer
      var oEkgrp = this.getView().byId("ekgrp");
      if(oEkgrp){
        utils.setInitInput(oEkgrp);
      }

      //2. Material
      var oMatnr = this.getView().byId("matnr");
      if(oMatnr){
        utils.setInitInput(oMatnr);
      }

      //3. Quantity 
      var oMenge = this.getView().byId("menge");
      if(oMenge){
        utils.setInitInput(oMenge);
      }

      //4 Unit
      var oMeins = this.getView().byId("meins");
      if(oMeins){
        utils.setInitInput(oMeins);
      }

      //5.Safra
      var oSafra = this.getView().byId("zzcttxt01");
      if(oSafra){
        utils.setInitInput(oSafra);
      }

      //6.Price 
      var oNetpr = this.getView().byId("netpr");
      if(oNetpr){
        utils.setInitInput(oNetpr);
      }

      //7. Currency
      var oWaers = this.getView().byId("waers");
      if(oWaers){
//        utils.setInitInput(oWaers);
        oWaers.setValueState("None");
      }
      ...
      var oWerks = this.getView().byId("werks");
      if(oWerks){
        utils.setInitInput(oWerks);
      }

      //10. Location(Warehouse)
      var oLgort = this.getView().byId("lgort");
      if(oLgort){
        utils.setInitInput(oLgort);
      }

      //11. Exp. Delivery Date
      var oEindt = this.getView().byId("eindt");
      if(oEindt){
        utils.setInitInput(oEindt);
      }

      //12. Vendor
      var oLifnr = this.getView().byId("lifnr");
      if(oLifnr){
        utils.setInitInput(oLifnr);
      }

      //13. Payment Date
      var oSlfdt = this.getView().byId("slfdt");
      if(oSlfdt){
        utils.setInitInput(oSlfdt);
      }

      //14. Region
      var oZzaddi1= this.getView().byId("zzaddi1");
      if(oZzaddi1){
        utils.setInitInput(oZzaddi1);
      }

      //15. Incoterms
      var oInco1= this.getView().byId("inco1");
      if(oInco1){
        utils.setInitInput(oInco1);
      }
      var oInco2L = this.getView().byId("inco2l");
      if(oInco2L){
        utils.setInitInput(oInco2L);
      }

      //16. Tax Code
      var oMwskz= this.getView().byId("mwskz");
      if(oMwskz){
        utils.setInitInput(oMwskz);
      }

      // Agente 
      var oZblc = this.getView().byId("zblc");
      if(oZblc){
        utils.setInitInput(oZblc);
      }
      var oZblcWaers = this.getView().byId("zblcWaers");
      if(oZblcWaers){
//        utils.setInitInput(oZblcWaers);
        oWaers.setValueState("None");
      }

      var oZfc4 = this.getView().byId("zfc4");
      if(oZfc4){
        utils.setInitInput(oZfc4);
      }
      var oZfc4Waers = this.getView().byId("zfc4Waers");
      if(oZfc4Waers){
//        utils.setInitInput(oZfc4Waers);
        oZfc4Waers.setValueState("None");
      }

      var oZbal = this.getView().byId("zbal");
      if(oZbal){
        utils.setInitInput(oZbal);
      }
      var oZbalWaers = this.getView().byId("zbalWaers");
      if(oZbalWaers){
//        utils.setInitInput(oZbalWaers);
        oZbalWaers.setValueState("None");
      }
    },
